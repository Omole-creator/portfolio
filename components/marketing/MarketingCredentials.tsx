...
export function MarketingCredentials() {
  return (
    <section className="border-t border-line bg-white py-24 md:py-32">
      <div className="container-x">
        ...
        <div className="mt-14 grid gap-10 lg:grid-cols-2">
          <Reveal>
            <SkillList title="Technical skills" items={marketingTechnicalSkills} />
          </Reveal>
          <Reveal delay={0.05}>
            <SkillList title="Soft skills" items={marketingSoftSkills} />
          </Reveal>
        </div>
        ...
        <Reveal delay={0.1}>
          <div className="mt-12">
            ...
            <ul className="mt-4 grid gap-3 sm:grid-cols-2">
              {marketingCertifications.map((cert) => (
                <li
                  key={cert.name}
        ...
        <Reveal delay={0.15}>
          <div className="mt-12 flex flex-col items-start justify-between gap-6 rounded-2xl bg-navy p-8 text-white sm:flex-row sm:items-center">
            <div>
              <p className="eyebrow text-gold">{marketingResume.eyebrow}</p>
              <h3 className="mt-2 text-2xl font-semibold">{marketingResume.title}</h3>
            </div>
            <TrackedLink
              href={marketingResume.href}
              cta="download-cv"
              download="Omole-Usuangbon-Creative-Marketing-CV.pdf"
